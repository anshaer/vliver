document.addEventListener("DOMContentLoaded", () => {
    fetch('data.json')
        ...
        .then(data => {
            ...
            // 輔助函式：渲染清單 (推廣與合作夥伴共用)
            const renderPromoLikeSection = (containerId, items) => {
                    ...
                    promoItem.innerHTML = `
                        ...
                        <div class="promo-content">
                            ...
                            <div class="promo-links">
                                ${item.ytUrl ? `<a href="${item.ytUrl}" class="promo-link-item" target="_blank"><i class="fab fa-youtube"></i> ${item.channel1Name}</a>` : ''}
                                ${item.xUrl ? `<a href="${item.xUrl}" class="promo-link-item" target="_blank"><i class="fa-solid fa-hashtag"></i> ${item.channel2Name}</a>` : ''}
                            </div>
                        </div>
                    `;
            ...
            // 2. 渲染推廣與合作夥伴
            renderPromoLikeSection("promo-list", data.promo);
            renderPromoLikeSection("partner-list", data.partner);
