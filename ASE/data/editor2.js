/**
 * 專業動態圖片專案製作器 - 核心控制邏輯腳本 (editor.js - 擴充 APNG/AVIF 導出版)
 */
...
function deleteLayer(index) {
    if(project.images[index].id === selectedImageId) { selectedImageId = null; propertyPanel.style.display = 'none'; drawFrame(); return; }
...
document.getElementById('btn-export-webm').addEventListener('click', async () => {
    ...
    saveBlobToDisk(finalBlob, "animation_output.webm");
    isRecording = false; btnWebm.textContent = backup; btnWebm.disabled = false; changeCurrentNode(0);
});

// 💡 3. 新增功能：匯出高畫質透明 APNG 動畫
document.getElementById('btn-export-apng').addEventListener('click', async () => {
    if(project.images.length === 0) { alert("請上傳素材！"); return; }
    const btnApng = document.getElementById('btn-export-apng'); const backup = btnApng.textContent;
    btnApng.textContent = "編碼中..."; btnApng.disabled = true;
    document.getElementById('btn-stop').click(); isRecording = true;

    let queue = generateFrameQueue();
    let framesArray = [];
    let delaysArray = [];
    const frameDelayMs = project.nodeDuration * 1000;

    // 依序渲染影格並抓取 ImageData 記憶體資料流
    for(let f of queue) {
        currentNode = f;
        drawFrame();
        const fData = ctx.getImageData(0, 0, canvas.width, canvas.height);
        framesArray.push(fData.data.buffer); // 壓入 RGBA ArrayBuffer
        delaysArray.push(frameDelayMs);
    }

    // 調用 UPNG.js 核心進行無損 APNG 陣列封裝 (0 代表不失真原色數)
    const apngBuffer = UPNG.encode(framesArray, canvas.width, canvas.height, 0, delaysArray);
    const finalBlob = new Blob([apngBuffer], { type: "image/apng" });

    saveBlobToDisk(finalBlob, "animation_output.apng");
    isRecording = false; btnApng.textContent = backup; btnApng.disabled = false; changeCurrentNode(0);
});

// 💡 4. 新增功能：匯出原生硬體級 AVIF 動畫
document.getElementById('btn-export-avif').addEventListener('click', async () => {
    if(project.images.length === 0) { alert("請上傳素材！"); return; }
    const btnAvif = document.getElementById('btn-export-avif'); const backup = btnAvif.textContent;
    btnAvif.textContent = "AV1錄製..."; btnAvif.disabled = true;
    document.getElementById('btn-stop').click(); isRecording = true;

    let queue = generateFrameQueue();
    const stream = canvas.captureStream(10);
    
    // 自動探測並提取瀏覽器內建最頂級的 AV1 開源硬體加速解算核心
    let av1Format = ['video/webm;codecs=av1', 'video/mp4;codecs=av1', 'video/webm'].find(f => MediaRecorder.isTypeSupported(f));
    const recorder = new MediaRecorder(stream, { mimeType: av1Format });
    const chunks = []; recorder.ondataavailable = (e) => { if (e.data && e.data.size > 0) chunks.push(e.data); };

    const finishPromise = new Promise(r => recorder.onstop = () => r(new Blob(chunks, { type: 'image/avif' })));
    recorder.start();
    ...
    saveBlobToDisk(finalBlob, "animation_output.avif");
    isRecording = false; btnAvif.textContent = backup; btnAvif.disabled = false; changeCurrentNode(0);
});

// 共同輔助模組：影格佇列生成器
function generateFrameQueue() {
    let q = [];
    for(let i=0; i<project.totalNodes; i++) q.push(i);
    if(project.loopType === 'pingpong') { for(let i=project.totalNodes-2; i>=1; i--) q.push(i); }
    return q;
}
